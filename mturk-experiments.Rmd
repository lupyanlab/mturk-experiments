...
- **Go to requester.wisc.edu**, find or create the HIT, and insert JavaScript code that fetches the current worker's ID and appends it to the Qualtrics survey URL.
- **Go to qualtrics.com**, and configure the Qualtrics survey to fetch the workerId from the URL and add it to the data.
...
qualtricsUrl = qualtricsUrl + "?workerId=" + urlVars["workerId"] + "&assignmentId=" + urlVars["assignmentId"] + "&hitId=" + urlVars["hitId"];

document.write("<a href=" + qualtricsUrl + ">Click here to take the survey.</a>");
```
